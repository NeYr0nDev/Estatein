import Image from 'next/image';
import Link from 'next/link';
import { useCurrency } from './CurrencyContext';

// Описываем, какие данные будет принимать карточка
interface PropertyCardProps {
  id: string;
  imageSrc: string;
  title: string;
  description: string;
  bedrooms: number;
  bathrooms: number;
  type: string;
  price: string;
}

export default function PropertyCard({
  id,
  imageSrc,
  title,
  description,
  bedrooms,
  bathrooms,
  type,
  price
}: PropertyCardProps) {
  const { formatPrice } = useCurrency();
  const formattedPrice = formatPrice(Number(String(price).replace(/\D/g, "")));
  return (
    // h-full и flex-col нужны, чтобы карточки в свайпере были одинаковой высоты
    <div className="flex flex-col p-6 bg-grey-08 border border-grey-15 rounded-xl h-full">
      
      {/* 1. Изображение */}
      {/* aspect-[4/3] сохраняет пропорции фото независимо от ширины экрана */}
      <div className="relative w-full aspect-4/3 rounded-xl overflow-hidden mb-4">
        <Image 
          src={imageSrc} 
          alt={title} 
          fill 
          className="object-cover"
        />
      </div>

      {/* 2. Заголовок и описание */}
      <h3 className="text-xl font-semibold text-white mb-0.5">{title}</h3>
      <p className="text-grey-50 text-sm mb-5 leading-relaxed">
        {description}{' '}
        <button className="text-white underline font-medium">
          Читать дальше
        </button>
      </p>

      {/* 3. Теги (Спальни, Ванные, Тип) */}
      <div className="flex flex-wrap gap-2.5 mb-6">
        <div className="flex items-center gap-1.5 px-3 py-1.5 bg-grey-10 border border-grey-15 rounded-full">
          {/* Иконка кровати */}
          <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            <g clip-path="url(#clip0_139_6526)">
            <g clip-path="url(#clip1_139_6526)">
            <path d="M10.0119 17.4597C7.01369 17.4597 4.01627 17.4597 1.01806 17.4597C0.714934 17.4597 0.574786 17.5984 0.574786 17.8977C0.574786 17.9752 0.578723 18.0527 0.573999 18.1295C0.5677 18.2225 0.558252 18.3155 0.54093 18.4062C0.476368 18.7372 0.159068 19.0023 -0.220432 18.9984C-0.601507 18.9946 -0.917233 18.7116 -0.974709 18.3481C-0.988094 18.2628 -0.99518 18.1752 -0.99518 18.0891C-0.995967 16.3279 -1.00778 14.5659 -0.991243 12.8046C-0.981795 11.7783 -0.602295 10.8853 0.125212 10.1419C0.566913 9.69069 1.09207 9.36976 1.69282 9.16511C2.16522 9.00465 2.65338 8.95581 3.15098 8.95581C7.76246 8.95736 12.3739 8.95348 16.9854 8.95891C18.3365 8.96046 19.4309 9.4969 20.2419 10.5636C20.6403 11.0884 20.8781 11.6845 20.9592 12.3357C20.9867 12.555 20.9985 12.7783 20.9993 13C21.0025 14.7101 21.0017 16.4202 21.0009 18.1302C21.0009 18.3977 20.934 18.6411 20.7104 18.8178C20.4741 19.0046 20.2104 19.0535 19.9293 18.9395C19.66 18.8295 19.49 18.6271 19.4514 18.338C19.4317 18.1922 19.4341 18.0426 19.4301 17.8953C19.4238 17.6574 19.3238 17.5139 19.1239 17.4698C19.0798 17.4597 19.0333 17.4605 18.9876 17.4605C15.9965 17.4605 13.0054 17.4605 10.0135 17.4605L10.0119 17.4597Z" fill="white"/>
            <path d="M9.90246 2C11.8472 2 13.7912 2 15.7351 2C16.3862 2 16.9949 2.14341 17.52 2.53953C18.1956 3.04884 18.59 3.71938 18.6231 4.56124C18.6514 5.27519 18.6388 5.9907 18.642 6.70543C18.6444 7.16589 18.642 7.62636 18.642 8.08682C18.642 8.12791 18.6396 8.17132 18.6286 8.21085C18.5948 8.33566 18.5263 8.38527 18.3971 8.36124C18.2562 8.33488 18.12 8.27829 17.979 8.26279C17.6476 8.22713 17.3153 8.20233 16.9823 8.18527C16.7602 8.17364 16.6917 8.13411 16.6602 7.91938C16.5595 7.23876 15.9194 6.62636 15.1005 6.63566C14.1872 6.64574 13.2739 6.63798 12.3613 6.63798C11.6819 6.63798 11.1016 7.03643 10.8709 7.66434C10.8378 7.75426 10.8252 7.85194 10.8079 7.94651C10.7756 8.11628 10.7119 8.17984 10.5402 8.18217C10.1804 8.18605 9.81979 8.18605 9.45997 8.18217C9.29463 8.18062 9.22928 8.11628 9.19936 7.92791C9.15763 7.66357 9.0537 7.42791 8.88364 7.22093C8.57579 6.84574 8.17739 6.64186 7.68609 6.63953C6.74363 6.63488 5.80118 6.63721 4.85794 6.63876C4.06587 6.63953 3.44623 7.24884 3.34545 7.91783C3.31317 8.13411 3.24782 8.17209 3.02579 8.18605C2.6581 8.21008 2.29041 8.24419 1.92351 8.28295C1.81092 8.29457 1.70305 8.34264 1.59125 8.36124C1.47078 8.38062 1.41488 8.34031 1.38103 8.22403C1.36764 8.17829 1.36134 8.12946 1.36134 8.0814C1.36055 6.97674 1.3456 5.87209 1.36449 4.76744C1.38418 3.61163 1.92666 2.76202 2.988 2.24806C3.40844 2.04574 3.86431 2 4.3257 2C6.18462 2 8.04354 2 9.90246 2Z" fill="white"/>
            </g>
            </g>
            <defs>
            <clipPath id="clip0_139_6526">
            <rect width="20" height="20" fill="white"/>
            </clipPath>
            <clipPath id="clip1_139_6526">
            <rect width="22" height="17" fill="white" transform="translate(-1 2)"/>
            </clipPath>
            </defs>
            </svg>
          <span className="text-white text-sm">{bedrooms}-Спальни</span>
        </div>
        
        <div className="flex items-center gap-1.5 px-3 py-1.5 bg-grey-10 border border-grey-15 rounded-full">
          {/* Иконка ванны */}
          <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            <g clip-path="url(#clip0_139_6532)">
            <g clip-path="url(#clip1_139_6532)">
            <path d="M0.269455 13.7616H10.11V15.5697H16.2615V13.7655H18.7054C18.8637 15.9158 18.8761 17.8304 16.6707 19.2897C16.6437 19.3075 16.6221 19.3318 16.5623 19.385C16.8335 19.6103 17.1009 19.8329 17.3655 20.0529C17.0082 20.3945 16.7212 20.669 16.3922 20.9842C16.1019 20.6848 15.7879 20.3373 15.4458 20.0207C15.3453 19.9275 15.1673 19.8894 15.0215 19.8802C14.7831 19.8657 14.5414 19.9045 14.301 19.9052C11.1057 19.9071 7.91048 19.9065 4.71457 19.9065C4.56154 19.9065 4.398 19.936 4.25679 19.8933C3.83054 19.7653 3.55994 19.9629 3.30642 20.2684C3.09034 20.5284 2.84405 20.7629 2.61877 21.0007C2.31468 20.6808 2.03949 20.3918 1.73474 20.0707C1.939 19.89 2.19843 19.6602 2.49004 19.4014C2.28906 19.2576 2.13866 19.1577 1.99614 19.0474C0.928209 18.2244 0.350896 17.1329 0.275366 15.7924C0.23793 15.1284 0.268798 14.4604 0.268798 13.7616H0.269455Z" fill="white"/>
            <path d="M0.275543 8.78712C0.270288 8.66825 0.261093 8.56119 0.261093 8.45413C0.260437 6.71366 0.258466 4.97384 0.261093 3.23336C0.26372 1.32081 1.59108 -0.0137781 3.50101 1.43622e-05C4.14203 0.00461185 4.80275 -0.00852383 5.41882 0.136625C6.5117 0.394741 7.20001 1.15267 7.53235 2.2206C7.59671 2.42814 7.68012 2.52863 7.90803 2.59037C9.19401 2.93912 10.0964 4.14104 10.1128 5.48219C10.1155 5.69827 10.1128 5.91435 10.1128 6.14882H4.02578C3.73548 4.61392 4.56959 3.02976 6.31335 2.5352C6.15573 1.85017 5.48121 1.2722 4.7732 1.24725C4.23792 1.2282 3.70067 1.22623 3.16539 1.24725C2.24261 1.28337 1.50044 2.13062 1.4965 3.14666C1.48994 4.89765 1.49453 6.64929 1.49453 8.40028C1.49453 8.51981 1.49453 8.63935 1.49453 8.78647H0.274229L0.275543 8.78712Z" fill="white"/>
            <path d="M10.0931 12.5327C9.96436 12.5327 9.86781 12.5327 9.77126 12.5327C6.64366 12.5327 3.5154 12.5334 0.387794 12.5321C-0.285409 12.5321 -0.769459 12.2135 -0.939566 11.6697C-1.19571 10.8514 -0.615771 10.0632 0.268259 10.0514C1.20877 10.0389 2.14929 10.0481 3.0898 10.0481C5.29922 10.0481 7.50799 10.0481 9.71741 10.0481C9.83563 10.0481 9.95385 10.0481 10.0931 10.0481V12.5321V12.5327Z" fill="white"/>
            <path d="M15.0237 14.3711H11.3634C11.3575 14.2555 11.3477 14.1504 11.3477 14.0453C11.3464 12.7771 11.3464 11.5082 11.3477 10.2399C11.3483 9.32239 11.8494 8.81798 12.763 8.81404C13.1019 8.81272 13.4408 8.8055 13.7797 8.81601C14.5409 8.84031 15.0381 9.35194 15.0401 10.1151C15.044 11.4385 15.0414 12.7613 15.0401 14.0847C15.0401 14.1708 15.0309 14.2561 15.0237 14.3711Z" fill="white"/>
            <path d="M16.2832 12.5209V10.0481C16.8782 10.0481 17.4648 10.0462 18.0506 10.0488C18.356 10.0501 18.668 10.0251 18.9668 10.0744C19.619 10.1821 20.0433 10.7161 20.0164 11.3394C19.9901 11.9508 19.5389 12.4756 18.8985 12.5091C18.0394 12.5544 17.1764 12.5203 16.2839 12.5203L16.2832 12.5209Z" fill="white"/>
            </g>
            </g>
            <defs>
            <clipPath id="clip0_139_6532">
            <rect width="20" height="20" fill="white"/>
            </clipPath>
            <clipPath id="clip1_139_6532">
            <rect width="21.0171" height="21" fill="white" transform="translate(-1)"/>
            </clipPath>
            </defs>
            </svg>

          <span className="text-white text-sm">{bathrooms}-Ванные</span>
        </div>
        
        <div className="flex items-center gap-1.5 px-3 py-1.5 bg-grey-10 border border-grey-15 rounded-full">
          {/* Иконка виллы/здания */}
          <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path fill-rule="evenodd" clip-rule="evenodd" d="M4 16.5V3.5H3.75C3.33579 3.5 3 3.16421 3 2.75C3 2.33579 3.33579 2 3.75 2H16.25C16.6642 2 17 2.33579 17 2.75C17 3.16421 16.6642 3.5 16.25 3.5H16V16.5H16.25C16.6642 16.5 17 16.8358 17 17.25C17 17.6642 16.6642 18 16.25 18H12.75C12.3358 18 12 17.6642 12 17.25V14.75C12 14.3358 11.6642 14 11.25 14H8.75C8.33579 14 8 14.3358 8 14.75V17.25C8 17.6642 7.66421 18 7.25 18H3.75C3.33579 18 3 17.6642 3 17.25C3 16.8358 3.33579 16.5 3.75 16.5H4ZM7 5.5C7 5.22386 7.22386 5 7.5 5H8.5C8.77614 5 9 5.22386 9 5.5V6.5C9 6.77614 8.77614 7 8.5 7H7.5C7.22386 7 7 6.77614 7 6.5V5.5ZM7.5 9C7.22386 9 7 9.22386 7 9.5V10.5C7 10.7761 7.22386 11 7.5 11H8.5C8.77614 11 9 10.7761 9 10.5V9.5C9 9.22386 8.77614 9 8.5 9H7.5ZM11 5.5C11 5.22386 11.2239 5 11.5 5H12.5C12.7761 5 13 5.22386 13 5.5V6.5C13 6.77614 12.7761 7 12.5 7H11.5C11.2239 7 11 6.77614 11 6.5V5.5ZM11.5 9C11.2239 9 11 9.22386 11 9.5V10.5C11 10.7761 11.2239 11 11.5 11H12.5C12.7761 11 13 10.7761 13 10.5V9.5C13 9.22386 12.7761 9 12.5 9H11.5Z" fill="white"/>
            </svg>

          <span className="text-white text-sm">{type}</span>
        </div>
      </div>

      {/* 4. Подвал (Цена и кнопка) */}
      <div className="flex flex-col 2xl:flex-row items-start 2xl:items-center justify-between mt-auto w-full pt-4 gap-4">
        
        {/* Добавили min-w-0, чтобы защита от вылезания текста (truncate) работала на 100% */}
        <div className="flex flex-col min-w-0 max-w-full">
          <span className="text-grey-50 text-sm mb-1">Цена</span>
          <span className="text-xl font-semibold text-white truncate" title={formattedPrice}>
            {formattedPrice}
          </span>
        </div>
        
        {/* Заменили xl на 2xl */}
        <Link 
          href={`/Properties/${id}`} 
          className="w-full 2xl:w-auto text-center px-4 py-3.5 bg-purple-60 hover:bg-purple-65 transition-colors rounded-lg text-white text-sm font-medium shrink-0"
        >
          Подробная информация
        </Link>
      </div>

    </div>
  );
}