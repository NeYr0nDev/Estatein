...
export default function Properties() {
  ...
  const filters = [
    ...
    },
    {
      ...
      options: ["Вилла", "Квартира", "Коттедж", "Особняк"], // Исправил опечатку "Котедж"
      icon: (
        <svg
          width="20"
          height="20"
    ...
    },
    {
      ...
      icon: (
        ...
        >
          <path
            d="M10 6.25C8.96447 6.25 8.125 7.08947 8.125 8.125C8.125 9.16053 8.96447 10 10 10C11.0355 10 11.875 9.16053 11.875 8.125C11.875 7.08947 11.0355 6.25 10 6.25Z"
            fill="#999999"
          />
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M1.25 4.0625C1.25 3.19956 1.94956 2.5 2.8125 2.5H17.1875C18.0504 2.5 18.75 3.19956 18.75 4.0625V12.1875C18.75 13.0504 18.0504 13.75 17.1875 13.75H2.8125C1.94955 13.75 1.25 13.0504 1.25 12.1875V4.0625ZM6.875 8.125C6.875 6.39911 8.27411 5 10 5C11.7259 5 13.125 6.39911 13.125 8.125C13.125 9.85089 11.7259 11.25 10 11.25C8.27411 11.25 6.875 9.85089 6.875 8.125ZM15.625 7.5C15.2798 7.5 15 7.77982 15 8.125V8.13125C15 8.47643 15.2798 8.75625 15.625 8.75625H15.6313C15.9764 8.75625 16.2563 8.47643 16.2563 8.13125V8.125C16.2563 7.77982 15.9764 7.5 15.6313 7.5H15.625ZM3.75 8.125C3.75 7.77982 4.02982 7.5 4.375 7.5H4.38125C4.72643 7.5 5.00625 7.77982 5.00625 8.125V8.13125C5.00625 8.47643 4.72643 8.75625 4.38125 8.75625H4.375C4.02982 8.75625 3.75 8.47643 3.75 8.13125V8.125Z"
            fill="#999999"
          />
          <path
            d="M1.875 15C1.52982 15 1.25 15.2798 1.25 15.625C1.25 15.9702 1.52982 16.25 1.875 16.25C6.37504 16.25 10.7335 16.8518 14.8748 17.9791C15.867 18.2491 16.875 17.514 16.875 16.4626V15.625C16.875 15.2798 16.5952 15 16.25 15H1.875Z"
            fill="#999999"
          />
        </svg>
      ),
  ...
  const filteredProperties = // Умная фильтрация с искусственной задержкой для скелетонов
    useEffect(() => {
      ...
      const timer = setTimeout(() => {
        const results = propertiesData.filter((property) => {
          ...
          if (searchTerm) {
            ...
            const matchesDesc = property.description
              .toLowerCase()
              .includes(lowerSearch);
          ...
          // 1. Локация
          if (selectedValues[1] && property.location !== selectedValues[1])
            return false;

          // 2. Тип недвижимости
          if (selectedValues[2] && property.type !== selectedValues[2])
            return false;

          // 3. Ценовой диапазон
          if (selectedValues[3]) {
            const price = Number(String(property.price).replace(/\D/g, ""));
            const range = selectedValues[3];
            if (range === "₽5M - ₽25M" && (price < 5000000 || price > 25000000))
              return false;
            if (
              range === "₽25M - ₽100M" &&
              (price < 25000000 || price > 100000000)
            )
              return false;
            if (range === "₽100M+" && price < 100000000) return false;
          }

          // 4. Площадь
          if (selectedValues[4]) {
            const area = Number(String(property.area).replace(/\D/g, ""));
            const range = selectedValues[4];
            if (range === "100-200 кв.м" && (area < 100 || area > 200))
              return false;
            if (range === "200-500 кв.м" && (area < 200 || area > 500))
              return false;
            if (range === "500+ кв.м" && area < 500) return false;
          }

          // 5. Дата постройки
          if (selectedValues[5]) {
            const year = Number(String(property.yearBuilt).replace(/\D/g, ""));
            const range = selectedValues[5];
            if (range === "2024-2026" && (year < 2024 || year > 2026))
              return false;
            if (range === "2020 - 2023" && (year < 2020 || year > 2023))
              return false;
            if (range === "2015 - 2020" && (year < 2015 || year > 2020))
              return false;
            if (range === "Before 2015" && year >= 2015) return false;
          }

          return true;
        });

        setDisplayedProperties(results); // Сохраняем результат
  ...
  // Скролл к результатам при клике на поиск
    ...
    <div className="flex flex-col gap-20 md:gap-30 lg:gap-37.5 overflow-hidden">
      ...
      >
        ...
        <div className="flex flex-col gap-4 pt-12.5 md:pl-20 md:pr-50 lg:pr-100 lg:pl-40.5 md:pt-25 md:pb-32.5 border-b border-grey-15 md:border-none pb-12.5">
          ...
          <motion.p
            variants={fadeInUp}
            className="text-sm lg:text-lg md:text-base text-grey-50 leading-relaxed"
          >
            ...
            тщательно подобранной коллекцией объектов...
          </motion.p>
        </div>
        ...
        >
          ...
          <div className="flex flex-row items-center w-full md:w-auto md:mx-20 p-2.5 pl-4 md:p-4 md:pl-5 lg:p-5 lg:pl-6 bg-grey-08 border border-grey-15 rounded-xl shadow-[0_0_0_4px_#191919] lg:rounded-t-2xl lg:rounded-b-none lg:border-b-0 lg:shadow-[0_0_0_10px_#191919]">
            <div className="flex-1 w-full px-3 md:px-4 py-2">
              <input
                type="text"
                placeholder="Искать недвижимость по названию или описанию..."
                value={searchTerm}
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 w-15 h-12 md:w-auto md:h-auto md:px-5 md:py-3.5 bg-purple-60 hover:bg-purple-65 transition-colors rounded-lg text-white font-medium text-sm flex-shrink-0 z-10 relative overflow-hidden group"
            >
          ...
          <div
            ref={filtersRef}
            className="w-full grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-5 p-5 md:p-2.5 bg-grey-10 rounded-xl relative z-20"
          >
            {filters.map((filter) => {
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <span
                      className={`text-sm font-medium border-l border-grey-15 pl-2.5 truncate transition-colors ${hasSelectedValue ? "text-white" : "text-grey-50 group-hover:text-white"}`}
                    >
                      {currentValue}
                    </span>
                  </div>
                  <div
                    className={`text-grey-50 w-fit p-1 rounded-full bg-grey-10 transition-all duration-300 flex-shrink-0 ${isOpen ? "rotate-180 text-white" : "group-hover:text-white"}`}
                  >
                    <svg
                      className="w-7 h-7"
                      width="18"
                  ...
                  <div
                    className={`absolute top-[calc(100%+8px)] left-0 w-full bg-grey-10 border border-grey-15 rounded-lg shadow-xl z-50 overflow-hidden transition-all duration-200 origin-top ${isOpen ? "opacity-100 scale-y-100" : "opacity-0 scale-y-0 pointer-events-none"}`}
                  >
                    <div className="flex flex-col py-2">
                      {filter.options.map((option, idx) => (
                        <div
                          key={idx}
                          onClick={(e) =>
                            handleSelectOption(filter.id, option, e)
                          }
      ...
      >
        ...
        <div className="flex flex-col gap-10 lg:gap-14 mt-3.5">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-5">
            <div className="flex flex-col gap-3.5">
              <motion.h1
                variants={fadeInUp}
                className="font-semibold text-[1.75rem] lg:text-5xl text-white"
              >
                Откройте для себя мир возможностей
              </motion.h1>
              <motion.p
                variants={fadeInUp}
                className="font-medium text-sm lg:text-lg text-grey-60 max-w-325"
              >
                Наш портфель недвижимости столь же разнообразен, как и ваши
                мечты. Найдите идеальный вариант с помощью удобных фильтров.
              </motion.p>
            </div>

            <div className="flex items-center gap-4 shrink-0">
              {/* Кнопка сброса фильтров */}
              {(Object.keys(selectedValues).length > 0 || searchTerm) && (
                <button
                  onClick={clearFilters}
                  className="px-5 py-3 text-sm font-medium text-white border border-grey-15 bg-grey-10 rounded-lg hover:border-purple-60 transition-colors"
                >
                  Сбросить
                </button>
              )}

              {/* ПЕРЕКЛЮЧАТЕЛЬ: Слайдер / Карта */}
              <div className="flex bg-grey-10 border border-grey-15 rounded-lg p-1">
                <button
                  onClick={() => setViewMode("slider")}
                  className={`px-4 py-2 text-sm font-medium rounded-md transition-colors ${viewMode === "slider" ? "bg-grey-15 text-white shadow-sm" : "text-grey-50 hover:text-white"}`}
                >
                  Списком
                </button>
                <button
                  onClick={() => setViewMode("map")}
                  className={`px-4 py-2 text-sm font-medium rounded-md transition-colors ${viewMode === "map" ? "bg-grey-15 text-white shadow-sm" : "text-grey-50 hover:text-white"}`}
                >
                  На карте
                </button>
              </div>
          ...
          <motion.div variants={fadeInSection}>
            {isLoading ? (
              ...
              viewMode === "slider" ? (
                ...
                // 3 РЕЖИМ КАРТЫ
                <div className="w-full h-[500px] lg:h-[600px] rounded-xl overflow-hidden border border-grey-15 bg-grey-10">
                  <YMaps>
                    <Map
                      defaultState={{ center: [20, 0], zoom: 2 }}
                      width="100%"
                      height="100%"
                    >
                      {displayedProperties.map((property) => (
              ...
              <div className="w-full flex flex-col items-center justify-center py-20 px-4 bg-grey-10 border border-grey-15 rounded-xl text-center gap-4">
                <div className="w-full flex flex-col items-center justify-center py-20 px-4 bg-grey-10 border border-grey-15 rounded-xl text-center gap-4">
                  <div className="w-16 h-16 rounded-full bg-grey-15 flex items-center justify-center mb-2">
                    <svg
                      width="24"
                      height="24"
                      viewBox="0 0 24 24"
                  ...
                  <p className="text-grey-60 max-w-md">
                    По вашим параметрам не удалось найти подходящую
                    недвижимость. Попробуйте изменить критерии поиска или
                    сбросить фильтры.
                  </p>
                  <button
                    onClick={clearFilters}
                    className="mt-4 px-6 py-3 bg-purple-60 hover:bg-purple-65 transition-colors text-white text-sm font-medium rounded-lg"
                  >
