...
export default function properties() {
  // Состояние для отслеживания открытой менюшки (храним id)
  const [openDropdown, setOpenDropdown] = useState<number | null>(null);
  // Состояние для хранения выбранных значений (ключ - id фильтра, значение - выбранный текст)
  const [selectedValues, setSelectedValues] = useState<Record<number, string>>(
    {},
  );

  // Реф для отслеживания кликов вне области фильтров
  const filtersRef = useRef<HTMLDivElement>(null);

  // Хук для закрытия менюшки при клике в любое другое место на экране
  useEffect(() => {
  ...
  const handleSelectOption = (
    ...
    e.stopPropagation(); // Останавливаем всплытие клика, чтобы менюшка не открылась снова
    setSelectedValues((prev) => ({ ...prev, [filterId]: option }));
    setOpenDropdown(null); // Закрываем после выбора
  };

  // Наши фильтры + добавлены массивы с опциями
  const filters = [
    {
      id: 1,
      label: "Локация",
  ...
  return (
    <div className="flex flex-col gap-20 md:gap-30 lg:gap-37.5">
      <section className="w-full flex flex-col gap-10 md:gap-14 bg-linear-116 from-0% from-grey-15 to-33% to-grey-08 px-4 md:px-0 md:border-b border-grey-15 relative md:mb-30">
        {/* БЛОК С ТЕКСТОМ */}
        ...
        <div className="w-full flex flex-col gap-4 lg:gap-0 md:px-20 md:absolute md:-bottom-32.5">
          ...
          <div className="flex flex-row items-center w-full md:w-auto md:mx-20 p-2.5 pl-4 md:p-4 md:pl-5 lg:p-5 lg:pl-6 bg-grey-08 border border-grey-15 rounded-xl shadow-[0_0_0_4px_#191919] lg:rounded-t-2xl lg:rounded-b-none lg:border-b-0 lg:shadow-[0_0_0_10px_#191919]">
            ...
            <button className="flex items-center justify-center gap-2 w-15 h-12 md:w-auto md:h-auto md:px-5 md:py-3.5 bg-purple-60 hover:bg-purple-65 transition-colors rounded-lg text-white font-medium text-sm flex-shrink-0 z-10 relative overflow-hidden group">
              {/* Анимация кнопки: пробегающий блик */}
          ...
          {/* Фильтры (Dropdowns) */}
          {/* Добавил ref, чтобы отслеживать клики вне этого блока */}
          <div
            ref={filtersRef}
            className="w-full grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-5 p-5 md:p-2.5 bg-grey-10 rounded-xl"
          >
            {filters.map((filter) => {
              const isOpen = openDropdown === filter.id;
              // Если значение выбрано - показываем его белым цветом, если нет - стандартный label серым
              const currentValue = selectedValues[filter.id] || filter.label;
              const hasSelectedValue = !!selectedValues[filter.id];
              return (
                // Добавили relative, чтобы выпадающее меню позиционировалось относительно этой плашки
                <div
                  key={filter.id}
                ...
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    {/* truncate обрезает слишком длинный текст и ставит троеточие */}
                    <span
                  ...
                  {/* Иконка стрелочки: переворачивается при открытии */}
                  <div
                  ...
                  {/* ВЫПАДАЮЩЕЕ МЕНЮ */}
                  {/* z-50 чтобы всегда было поверх остальных фильтров */}
                  <div
      ...
      <section className="flex flex-col w-full min-w-0 px-4 md:px-20 lg:px-40.5">
                <Image src="/svg/stars.svg" alt="alt" width={46} height={20} className="w-[2.875rem] h-auto" />
                <div className="flex flex-col gap-10 lg:gap-20">
                    <div className="flex flex-col gap-2">
                        <h1 className="font-semibold text-[1.75rem] lg:text-5xl text-white">Откройте для себя мир возможностей</h1>
                        <p className="font-medium text-sm lg:text-lg text-grey-60 max-w-325">Наш портфель недвижимости столь же разнообразен, как и ваши мечты. Ознакомьтесь со следующими категориями, чтобы найти идеальный вариант, соответствующий вашему представлению о доме.</p>
                    </div>
                ...
                <SwiperSlide>
                    <PropertyCard2
                      ...
                      description="Просыпайтесь под убаюкивающий шум волн. Эта вилла на берегу моря предлагает..."
                      price="1,200,000"
                    ></PropertyCard2>
                </SwiperSlide>
                ...
                <SwiperSlide>
                    <PropertyCard2
                      ...
                      title="Столичный рай"
                      description="Погрузитесь в атмосферу города. Эта современная квартира в самом центре..."
                      price="650,000"
                    ></PropertyCard2>
                </SwiperSlide>
                ...
                <SwiperSlide>
                    <PropertyCard2
                      ...
                      description="ПросНасладитесь спокойствием загородной жизни. Этот очаровательный коттедж расположен среди холмов..."
                      price="350,000"
                    ></PropertyCard2>
                </SwiperSlide>
                <SwiperSlide>
      ...
      <section className="flex flex-col w-full min-w-0 px-4 md:px-20 lg:px-40.5">
          ...
          <div className="flex flex-col gap-10 lg:gap-20">
              <div className="flex flex-col gap-2">
                  <h1 className="font-semibold text-[1.75rem] lg:text-5xl text-white">Давайте сделаем это</h1>
                  <p className="font-medium text-sm lg:text-lg text-grey-60 max-w-325">Готовы сделать первый шаг к приобретению недвижимости своей мечты? Заполните форму ниже, и наши специалисты по недвижимости сотворят чудо и найдут для вас идеальный вариант. Не ждите, давайте отправимся в это увлекательное путешествие вместе.</p>
              </div>
